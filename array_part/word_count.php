...
/**
 * Функция возвращает ассоциативный массив с количеством слов  в тексте
 * @param $text
 * @return array
 */
function getCountOfWordsInText($text): array
{
    $text = prepareText($text);
    $word_array = array_map("strtolower", explode(" ", $text));
    $unique_word_array = array_fill_keys($word_array, 0);
    foreach ($word_array as $value)
        $unique_word_array[$value]++;

    return $unique_word_array;
}
/**
 * Подготовка текста.
 * Функция убирает знаки припинания, лишние пробелы и т.д.
 * @param $text
 * @return string
 */
function prepareText($text): string
{
    ...
    $text = preg_replace("/(\. |,|!|\"|;|'s|[a-zA-Z]-[a-zA-Z]|\?)+/", ' ', $text);
